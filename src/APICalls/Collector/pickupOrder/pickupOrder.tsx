...
import {
  ...
  GET_ALL_LOGISTICS_PICK_UP_ORDER,
  GET_PICK_UP_ORDER_BY_ID,
  GET_PICK_UP_ORDER_DETAIL,
  UPDATE_PICK_UP_ORDER,
  UPDATE_PICK_UP_ORDER_DETAIL_STATUS,
  UPDATE_PICK_UP_ORDER_STATUS,
  GET_ALL_REASON,
  GET_3RDPARTY_LOGISTICLIST,
...
export const editPickupOrderStatus = async (
...
export const editPickupOrderDetailStatus = async (
  pickupOrderDtlId: string,
  data: PoDtlStatus
) => {
  const axiosConfig = Object.assign({}, UPDATE_PICK_UP_ORDER_DETAIL_STATUS);
  axiosConfig.url =
    UPDATE_PICK_UP_ORDER_DETAIL_STATUS.url + `/${pickupOrderDtlId}`;
...
export const getThirdPartyLogisticData = async () => {
  const auth = returnApiToken();
  try {
    const response = await axiosInstance({
      baseURL: window.baseURL.collector,
