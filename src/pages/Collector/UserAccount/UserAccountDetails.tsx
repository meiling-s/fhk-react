...
const UserAccountDetails: FunctionComponent<UserAccountDetailsProps> = ({
  ...
  const prohibitedLoginId: string[] = [
    "_astdadmin",
    "_superadmin",
    "_fhkadmin",
  ];
  ...
  const getExistingEmail = async () => {
    try {
      ...
      if (result) {
        ...
        let tempStaffID: string[] = [];
        const data = result.data.content;
        data.map((item: any) => {
          tempEmail.push(item.email);
          tempStaffID.push(item.staffId);
        });
        console.log(data, "data");

        setExistingEmail(tempEmail);
        setExistingStaffID(tempStaffID);
      }
    } catch (error: any) {
  ...
  useEffect(() => {
    const validate = async () => {
      ...
      userList?.some(
        (userLoginId) => userLoginId.toLowerCase() == loginId.toLowerCase()
      ) &&
        tempV.push({
          field: t("userAccount.loginName"),
      ...
      //   })
      staffIdList?.find(
        (userStaffID) => userStaffID.toLowerCase() == staffId.toLowerCase()
      ) &&
        tempV.push({
          field: t("userAccount.staffId"),
          problem: formErr.alreadyExist,
          type: "error",
        });
      contactNo?.toString() == "" &&
  ...
  const handleDuplicateErrorMessage = (input: string) => {
    const replacements: { [key: string]: string } = {
      "[Login ID]": t("userAccount.loginName"),
    };

    const matches = input.match(/\[(Login ID)\]/gi);
  ...
  const handleInvalidCharacterErrorMessage = (input: string) => {
    const replacements: { [key: string]: string } = {
      "[INVALID_LOGIN_ID_ERROR]": t("userAccount.loginName"),
    };

    const matches = input.match(/\[(INVALID_LOGIN_ID_ERROR)\]/gi);
  ...
  const handleCreateUser = async () => {
    ...
    if (validation.length === 0) {
      try {
        const response = await postUserAccount(formData);
        if (response) {
          onSubmitData();
        ...
        if (state.code === STATUS_CODE[400]) {
          const errorMessage = error.response.data.errorMessage;
          if (errorMessage.includes("[RESOURCE_DUPLICATE_ERROR]")) {
            setTrySubmited(true);
            handleDuplicateErrorMessage(errorMessage);
          } else if (errorMessage.includes("[INVALID_LOGIN_ID_ERROR]")) {
            setTrySubmited(true);
            handleInvalidCharacterErrorMessage(errorMessage);
          }
              ...
              <CustomField label={t("userAccount.staffId")}>
                <CustomTextField
                  ...
                  error={
                    ...
                    (trySubmited &&
                      validation.some(
                        (value) => value.field === t("userAccount.staffId")
                      ))
                  }
                />
              </CustomField>
            ...
            <CustomField label={t("userAccount.loginName")} mandatory>
              <CustomTextField
                ...
                error={
                  ...
                  (trySubmited &&
                    validation.some(
                      (value) => value.field === t("userAccount.loginName")
                    ))
                }
              />
            </CustomField>
