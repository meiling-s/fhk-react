...
const Inventory: FunctionComponent = () => {
    ...
    const initInventory = async () => {
        ...
        if (data) {
            ...
            data.content.map((item: any) => {
                ...
                const subType = recy
                    ? recy.recycSubType.find(
                          (sub) => sub.id == item.recycSubTypeId
                      )
                    : null;
    ...
    function getUniqueOptions(propertyName: keyof InventoryItem) {
        ...
        const options: Option[] = Array.from(optionMap.values()).map(
            (option) => ({
                value: option,
                label: option,
            })
    ...
    const handleSelectRow = (params: GridRowParams) => {
        ...
        selectedInv.inventoryDetail?.forEach((item) => {
            const pico = picoList.find(
                (pico) => pico.picoId == item.sourcePicoId
            );
            if (pico) {
