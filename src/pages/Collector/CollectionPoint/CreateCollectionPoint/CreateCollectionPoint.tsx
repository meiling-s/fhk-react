...
function CreateCollectionPoint() {
  ...
  const [contractList, setContractList] = useState<
    { contractNo: string; isEpd: boolean }[]
  >([]);
  ...
  useEffect(() => {
    ...
    colType == "" &&
      tempV.push({ field: "col.colType", problem: formErr.empty });
    siteType == "" &&
      tempV.push({ field: "col.siteType", problem: formErr.empty });
    colName == "" &&
      tempV.push({ field: "col.colName", problem: formErr.empty });
    address == "" &&
      tempV.push({ field: "col.address", problem: formErr.empty });
    dayjs(new Date()).isBetween(
      openingPeriod.startDate,
      openingPeriod.endDate
    ) &&
      status == false && //status == false: status is "CLOSED"
      tempV.push({ field: "col.openingDate", problem: "stillInPeriod" });
    premiseName == "" &&
      tempV.push({ field: "col.premiseName", problem: formErr.empty });
    premiseType == "" &&
      tempV.push({ field: "col.premiseType", problem: formErr.empty });
    recyclables.length == 0 &&
      tempV.push({ field: "col.recycType", problem: formErr.empty });
    console.log(
      "num:",
      staffNum,
    ...
    staffNum == "" &&
      tempV.push({ field: "col.numOfStaff", problem: formErr.empty });
    ...
    contractNo == "" &&
      tempV.push({ field: "col.contractNo", problem: formErr.empty });
    setValidation(tempV);
    console.log(tempV);
  }, [
    colType,
  ...
  const getValidationMsg = (valid: { field: string; problem: string }[]) => {
    ...
    if (wrongFormat.length > 0) {
      ...
      wrongFormat.map((w, index) => {
        tempM += t(w);
        tempM +=
          index == wrongFormat.length - 1 ? t("form.isInWrongFormat") : ", ";
      });
      if (tempM != "") {
        msg.push(tempM);
      }
    ...
    if (numSmallerThan0.length > 0) {
      ...
      numSmallerThan0.map((n, index) => {
        tempM += t(n);
        tempM +=
          index == numSmallerThan0.length - 1
            ? t("form.shouldNotSmallerThanZero")
            : ", ";
      });
      if (tempM != "") {
        msg.push(tempM);
